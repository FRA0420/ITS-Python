...
#     E' consentito 1 docente ogni 10 studenti. 
#     Il gruppo può avere almeno 1 docente, anche se ci sono meno di 10 studenti.
#     - add_student(student): Aggiunge uno studente al gruppo, 
#     solo se il limite per gli studenti non è stato raggiunto.
#     - add_lecturer(lecturer): Aggiunge un docente al gruppo, 
#     solo se il limite per i docenti non è stato raggiunto.
...
class Group:
    ...
    def add_student(self,student:Student):
        if len(self.students) < self.limit:
            ...
            student.set_group(self)   #riferito all'istanza corrrente di Group
        else:
            print(f"Impossibile aggiungere {student.name}: limite studenti raggiunto ({self.limit})")
    
    def add_lecturer(self,lecturer:Lecturer):
        if len(self.lecturers) < self.get_limit_lecturers():
            self.lecturers.append(lecturer)
        else:
            print(f"Impossibile aggiungere {lecturer.name}: limite docenti raggiunto ({self.get_limit_lecturers()})")
