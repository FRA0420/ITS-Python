...
# deve calcolare la dimensione compressa di tale file espressa come float 
# (ovvero deve calcolare l' 80% della dimensione non compressa), '
# 'calcolare il numero di blocchi (arrotondato al numero intero più vicino) da 512 byte necessari per la memorizzazione, '
# 'al fine di determinare se il file compresso può essere salvato nello spazio rimanente '
# 'nel supporto di memorizzazione o meno.
 
# In caso affermativo, il programma memorizza il file. 
# In tal caso, la funzione deve stampare la dimensione originale del file, 
...
# la cui dimensione compressa occupa un numero di blocchi più grande di quelli rimasti disponibili 
# sul supporto di memorizzazione. 
# In tal caso, la funzione deve avvisare l'utente che lo spazio disponibile sul supporto di memorizzazione 
...
def memorizza_file(files:list[int]):
    ...
    spazio_rimasto = 0
    for file in files:
        dim_compressa= file*0.8
        spazio_rimasto = dim_compressa / 512
        blocchi_usati = round(spazio_rimasto)
        if spazio_totale_blocchi >= blocchi_usati:
            spazio_totale_blocchi -= blocchi_usati
            print(f"File di {file} byte compresso in {dim_compressa} byte e memorizzato. Blocchi usati: {blocchi_usati}. Blocchi rimanenti: {spazio_totale_blocchi}. ")
        else:
